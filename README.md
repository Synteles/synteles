...
| File | Purpose |
|---|---|
| `.env.local` | Secrets and credentials (API keys, Keycloak passwords) |
| `config/platform.toml` | Platform model configuration (chat model, platform default models) |

`install.sh` creates `config/platform.toml` from the example automatically. Create `.env.local` manually:

```bash
cp .env.local.example .env.local
```

Then fill in the values relevant to your setup.
...
`model_id` is a [LiteLLM-supported model string](https://docs.litellm.ai/docs/providers). `secret_name` references the matching `PLATFORM_SECRET_*` credential in `.env.local` (see below).

#### Web search (Tavily)

Agentlets with web search capability need a Tavily API key. Get a free key at [app.tavily.com](https://app.tavily.com). Once set, the key is automatically available to every agentlet — no per-agentlet configuration needed.
...
Each model entry in `config/platform.toml` has a `secret_name` field. Add a matching `PLATFORM_SECRET_<SECRET_NAME>` entry in `.env.local` with the provider credentials as a JSON object:

```env
PLATFORM_SECRET_OPENAI={"OPENAI_API_KEY": "sk-..."}
PLATFORM_SECRET_AZURE_AI={"AZURE_API_KEY": "...", "api_base": "https://my-deployment.openai.azure.com"}
