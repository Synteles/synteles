...
def pytest_configure(config: object) -> None:
    """Create a stub platform.toml so model_catalog can be imported in CI.

    config/platform.toml is generated by install.sh and is gitignored. When
    pytest (or mocker.patch) imports tools.model_catalog, its module-level
    _load_platform_defaults() call raises FileNotFoundError if the file is
    absent. An empty TOML is valid: tomllib returns {} → no models loaded,
    but FileNotFoundError is still raised in production where install.sh
    hasn't been run.
    """
    toml_path = Path(__file__).resolve().parents[2] / "config" / "platform.toml"
