...
- `platform.toml` moved from `config/platform.toml` to the repository root — updated across Docker Compose volume mounts, all service path lookups (`synte-service`, `scheduler-service`), `synte-service/Dockerfile`, and documentation
- `install.sh` overhauled: brand-color terminal output, bordered step headers, braille spinner for the Docker build step, merge mode for re-running on an existing install, and an "Other" provider option with links to documentation; credential collection is now deferred — install generates placeholder JSON in `.env` and directs the user to fill in API keys manually rather than prompting interactively
- Docker Compose: shared YAML extension fields (`x-s3-env`, `x-oidc-env`, `x-backend-healthcheck`, `x-backend-deps`) eliminate duplicated environment and healthcheck blocks across `core-service`, `scheduler-service`, and `synte-service`; `AGENTLET_DURABLE_IMAGE` is now overridable via env var
- Nebius removed from the `install.sh` provider list and `.env.example`
...
- Turbopack disabled (`experimental: { turbo: false }`) in `ux-console` — eliminates intermittent missing-chunk build errors where RSC manifest references were emitted but corresponding `.js` chunk files were never written to disk; Webpack is used for all builds until the upstream emit bug is resolved
- `synte-service/Dockerfile`: stale `COPY config/ config/` replaced with `COPY platform.toml platform.toml` — fixes broken `docker compose build synte-service` after the `config/` directory was removed in a prior refactor
...
- `synte-service` CI: `pytest_configure` hook in `tests/conftest.py` creates a stub `platform.toml` before test collection so `tools.model_catalog` can be imported in environments where `install.sh` has not been run
