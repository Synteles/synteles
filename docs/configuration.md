...
Synteles uses two complementary configuration files at the repository root, both git-ignored and generated by `install.sh` on first setup:

**`.env`** holds secrets and deployment-specific credentials — API keys, passwords, encryption keys, and connection strings. It is consumed as environment variables injected into each service container at startup. Every field maps to a specific environment variable documented in the [Environment Variables](#environment-variables) section below.

**`platform.toml`** holds structured model configuration: which LLM powers the Synte chat assistant and which models are offered as platform defaults to agentlets. It contains no secrets — credentials are referenced by name via `secret_name` and resolved at runtime from the matching `PLATFORM_SECRET_*` variables in `.env`. Services read `platform.toml` directly from disk rather than via environment variables, so changes take effect on stack restart without re-injecting secrets.
...
- [platform.toml](#platformtoml)
  - [chat — Chat Engine](#chat--chat-engine)
  - [model — Platform Default Models](#model--platform-default-models)
...
Generated by `install.sh`. To set up manually:
...
Configures the LLM used by the Synte chat assistant.

| Field | Required | Default | Description |
|---|---|---|---|
| `model_id` | **Required** | `openai/gpt-4o` | [LiteLLM model string](https://docs.litellm.ai/docs/providers) for the chat assistant, e.g. `openai/gpt-4o`, `anthropic/claude-sonnet-4-6`, or `azure_ai/gpt-5.3-chat`. |
| `secret_name` | **Required** | — | Name of the matching `PLATFORM_SECRET_<NAME>` credential in `.env` that supplies the provider API keys. Case-insensitive; hyphens and spaces are normalised to underscores. |
...
The corresponding `.env` entry must provide the provider credentials as a JSON object:

```env
PLATFORM_SECRET_OPENAI={"OPENAI_API_KEY": "sk-..."}
```

### `[[model]]` — Platform Default Models

Each `[[model]]` entry defines a model made available to all agentlets that declare `secrets: [default]` in their YAML. Credentials are injected automatically at execution time from the matching `PLATFORM_SECRET_*` variable in `.env` — no per-agentlet API key configuration is needed.

Models without a matching `PLATFORM_SECRET_*` entry are silently skipped at execution time; the platform continues running with whichever models have valid credentials.

| Field | Required | Default | Description |
|---|---|---|---|
| `id` | **Required** | — | Internal identifier used in API responses. Must be unique across all model entries. |
| `label` | **Required** | — | Display name shown in the platform UI and model picker. |
| `provider` | **Required** | — | LiteLLM provider prefix, e.g. `openai`, `anthropic`, `azure_ai`, `bedrock`. |
| `model_id` | **Required** | — | Provider-specific model name passed to LiteLLM, e.g. `gpt-4o` or `claude-sonnet-4-6`. For Bedrock, the full ARN is accepted. |
| `secret_name` | **Required** | — | Key of the `PLATFORM_SECRET_*` credential in `.env` holding the provider API keys. Follows the same normalisation rules as `[chat].secret_name`. |
| `default_temperature` | **Required** | — | Temperature applied when the agentlet does not specify one. |
| `min_temperature` | Optional | — | Lower bound enforced at execution time if the agentlet requests a lower temperature. |
| `description` | Optional | — | Short description shown to users in the model picker. |
| `best_for` | Optional | — | List of use-case tags displayed alongside the model in the picker, e.g. `["coding & analysis", "document Q&A"]`. |
...
best_for            = ["general-purpose tasks", "coding & analysis"]
```
